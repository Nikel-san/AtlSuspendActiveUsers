...
The CSV must contain one email address per row, without a header. UTF-8 and UTF-8-BOM files are accepted; whitespace is stripped and blank rows are ignored. Date mode combines managed organization users from `GET /admin/v1/orgs/{org_id}/users` with Jira site users from `GET /rest/api/3/users/search` and product access group members from `GET /rest/api/3/group/member`, merging records by account ID. Organization responses use `links.next` pagination; site and group responses use paginated requests. Site and group users without a visible email are retained with `<account_id>@external.atlassian`; app principals, JSM portal customers, and IDs beginning with `qm:` are excluded because they cannot be suspended through organization APIs. File mode skips organization-user pagination and looks up each email through `GET /rest/api/3/user/search?query=...`. A populated `emailAddress` must match exactly, case-insensitively; if Atlassian redacts it, the returned search result is associated with the queried email. Managed accounts, including Jira `accountType="atlassian"`, are suspended through the lifecycle API; external/unmanaged and invited accounts use `POST /admin/v1/orgs/{org_id}/directory/users/{account_id}/suspend-access`. Invited accounts are selected only with `--include-invited`, including when never active. If lifecycle disable returns the verified-org-admin HTTP 403, suspension falls back to the organization directory endpoint. External profile checks tolerate HTTP 403 because those profiles are not organization-managed. HTTP 409 suspension responses are recorded as billing-administrator conflicts and do not stop later candidates. Accounts with an account ID receive a profile lookup before suspension so service-account job titles are checked consistently. Missing users are recorded as `User not found`.
